vis_AH_ggplot <- function(layout, key = NULL, 
  ...
  p1 <- 
    suppressWarnings({
      layout$edges %>% 
        ...
        geom_point(data = 
                     layout$vertices %>% 
                     mutate(level = forcats::fct_inorder(as.character(level), ordered = TRUE)), 
                   aes(x = x, y = y,
                       size = level,
                       fill = level,
  ...
  if(!is.null(key)) {
    ...
    p2b <- gginnards::extract_layers(p2, "GeomPath")
    
    output <- output %>% gginnards::append_layers(p2b, position = "bottom")
