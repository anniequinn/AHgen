vis_plotViolin <- function(results, 
  ...
  if(type == "half"){
    ...
    if(study == "flood") {
      ...
      # does this have to do with the fact that 'levels' are still attached to colsFloodRiver100, colsFloodRiver200?
    ...
    if(study == "offline") {
      ...
      violinPlot <-
        ...
        see::geom_violinhalf(
          data = results %>% filter(scenario == "Rawa Terate offline"), 
    ...
    if(study == "international") {
      
      violinPlot <-
        ggplot(data = results) +
        aes(x = levelName, y = value, fill = scenarioName) +
        see::geom_violinhalf(
          data = 
            results %>% 
            filter(scenarioName == "UK"), 
          alpha = 0.7, aes(fill = "#56B4E9"), flip = TRUE, linewidth = 0.1) + # using default colours from AHgen package; flipped
        see::geom_violinhalf(
          data = 
            results %>% 
            filter(scenarioName == "Indonesia"), 
          alpha = 0.7, aes(fill = "#85929E"), linewidth = 0.1) + # using default colours from AHgen package
        ggh4x::facet_grid2(
          independent = "x", levelName ~ location, 
          labeller = label_wrap_gen(width = 15), scales = "free", switch = "y") +
        ...
        scale_fill_manual(name = "country", 
                          values = levels(results$cols), 
                          labels = levels(results$scenarioName)) +
        scale_y_continuous(limits = c(0, NA), position = "right") +
        theme(axis.text.x = element_blank(),
              axis.text.y = element_text(color = "grey40", size = 24),
              axis.ticks.x = element_blank(),
    ...
    if(study == "international") {
      ...
      violinPlot <-
        ggplot(data = results) +
        aes(x = levelName, y = value, color = scenarioName) +
        geom_violin(alpha = 0.1, position = "identity", linewidth = 0.2) +
        ggh4x::facet_grid2(
          independent = "x", levelName ~ location, 
          labeller = label_wrap_gen(width = 15), scales = "free", switch = "y") +
        ...
        scale_color_manual(name = "country", 
                           values = levels(results$cols), 
                           labels = levels(results$scenarioName)) + # using default colours from AHgen package
        scale_y_continuous(limits = c(0, NA), position = "right") +
        guides(color = guide_legend(override.aes = list(linewidth = 2))) +
        theme(axis.text.x = element_blank(),
              axis.text.y = element_text(color = "grey40", size = 24),
