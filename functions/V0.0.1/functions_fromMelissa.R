...
genericAbstractionHierarchy <- 
  ...
  function(filename = "USAH.csv", nodeInfo = c("nodeName", "layer", "layerName", "filterCoverage",
                                               ...
                                               "subnetwork_medical", "subnetwork_lawOrder", "subnetwork_nature",
                                               "subnetwork_religion",	"subnetwork_infrastructure_transport",
                                               ...
                                               "subnetwork_infrastructure_waste", "subnetwork_infrastructure_water",
                                               ...
                                               "hazard_biological", "hazard_cyberAttack", "hazard_drought",
                                               "hazard_earthquake", "hazard_fire", "hazard_flood", "hazard_landslide",
                                               "hazard_snow", "hazard_wind")) {
...
weightLinks <- function(linkName = "links_2_VPM_to_GF", igraphlist, weights) {
  ...
  output <- igraph::union(igraphlist[[linkName]], weights)
...
baselinePhysicalObjects_fromFile_revised <- function(objectCountFilename, keyFilename = "OSMAHkey_CS.csv") { 
  
  if(str_detect(objectCountFilename, "RDS")) { 
    input <- readRDS(objectCountFilename) 
    input <- input$geotaggedDatabase %>% as_tibble %>% select(-geometry) %>% na.omit
  }
  if(str_detect(objectCountFilename, "csv")) { input <- read_csv(objectCountFilename, col_types = cols()) %>% na.omit }
  if(!str_detect(objectCountFilename, "csv") & !str_detect(objectCountFilename, "RDS")) { 
    stop("objectCountFilename must have an RDS or csv extension") 
...
results_networkSummary <- function(igraph) {
  ...
  funs <- list(nNodes = vcount, nEdges = ecount, assortativityDegree = assortativity_degree, diameter = diameter)
