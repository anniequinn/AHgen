...
```{r}
# Weight the original edge list with edgesNew
de_new = weight_edges(edgelist = de, edgesNew = de_w, remove = FALSE)
...
```{r}
# Weight the original edge list with edgesNew
de_new = weight_edges(edgelist = de, edgesNew = de_w, remove = TRUE)

# Inspect to confirm the level 4 node has been removed
de_new %>% filter(from == "Treat water" | to == "Treat water")
```
In both cases we can see that the vertex *Treat water* would still be present in the Urban Systems Abstraction Hierarchy. However, it would have all upward edges to level 3, and no downward edges to level 5, making it a "hanging" vertex.
