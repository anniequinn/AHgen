...
knitr::kable(metricsTable) %>% kable_styling(font_size = 8)
```
...
# Generate results
results = 
  gen_results(igraph = di, vInfo = dv, 
...
The function `gen_AH` uses multiple core functions to create a single standardised `AHgen` output. This creates a list of elements:
